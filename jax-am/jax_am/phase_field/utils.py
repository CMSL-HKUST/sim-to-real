...
class Field:
    ...
    """
    def __init__(self, pf_args, neper_path, ori2=None):
        self.pf_args = pf_args
        self.ori2 = ori2
        self.process_neper_mesh(neper_path)
        self.get_unique_ori_colors()

    def process_neper_mesh(self, neper_path):
        print(f"Processing neper mesh...")
        # neper_folder = os.path.join(self.pf_args['data_dir'], "neper")
        neper_folder = neper_path
...
def generate_new_mesh(pf_args, pf_args_layer, neper_path, polycrystal, layer, ori2):
    """
    Generate one new layer to model multi-layer polycrystal
    """
    polycrystal_next = Field(pf_args_layer, neper_path, ori2)
    # put the new layer upon the old one
    polycrystal_next.mesh.points[:, 2] = pf_args['domain_z'] + pf_args_layer['domain_z']*(layer-1) + polycrystal_next.mesh.points[:, 2]
    polycrystal_next.mesh.cells[0].data = len(polycrystal.mesh.points) + polycrystal_next.mesh.cells[0].data
    polycrystal_next.centroids[:, 2] = pf_args['domain_z'] + pf_args_layer['domain_z']*(layer-1) + polycrystal_next.centroids[:, 2]
    # merge points
