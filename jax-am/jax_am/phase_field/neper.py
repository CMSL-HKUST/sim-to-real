...
def pre_processing(pf_args, neper_path):
    ...
    if not os.path.exists(os.path.join(neper_path, 'domain.msh')):
        ...
        # os.system(f"neper -M -order 1 -cl {pf_args['mesh_size']} -elttype hex -faset faces {neper_path}/domain.tess")
        os.system(f"neper -M -order 1 -cl 0.000002 -elttype hex -faset faces {neper_path}/domain.tess")
    else:
